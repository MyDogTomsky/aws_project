# Using Apache, phpMyAdmin to enable static web sites to be Dynmaic
...
# Checking the being grouped if MySQL-related apps are installed.
...
##              enables Dynamic Function and the Application of changed specific Configuration.
...
# Moving to html to deploy the web files under /var/www/html
cd /var/www/html
...
#   .env  ==> APP_URL = load balancer / DB_HOST = RDS Endpoint  / DB_name & RDS Mater ID,pwd //
sudo vi .env
